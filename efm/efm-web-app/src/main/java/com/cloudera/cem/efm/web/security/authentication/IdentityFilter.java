 ...
 */
package com.cloudera.cem.efm.web.security.authentication;

import com.cloudera.cem.efm.web.security.authentication.exception.KnoxAuthenticationException;
import com.cloudera.cem.efm.web.security.authentication.knox.KnoxUtils;
...
public class IdentityFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        } catch (KnoxAuthenticationException kae) {
            // We need to redirect here because otherwise the exception will get swallowed in the catch block below,
            // and by the time we reach the http401AuthenticationEntryPoint, the exception will no longer be a Knox
            // exception because Spring security will produce an exception for not finding any credentials in the request
            final HttpServletRequest httpRequest = (HttpServletRequest) servletRequest;
            final HttpServletResponse httpResponse = (HttpServletResponse) servletResponse;
            KnoxUtils.sendRedirectToKnox(httpRequest, httpResponse, kae);
            return;
        } catch (Exception e) {
