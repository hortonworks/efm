...
/**
 * DeselectableBehavior constructor.
 *
 * @constructor
 */
function DeselectableBehavior() {
    var self = this;
    var canvas = null;
     ...
     * @param g
     */
    this.deselect = function (g) {
        // only need to update selection if necessary
        if (g.classed('selected')) {
            // we are currently selected, if shift key then deselect
        ...
        // deselect handled... stop propagation
    ...
    this.activate = function (components) {
        components.on('mousedown.deselection', function () {
            // get the clicked component to update selection
            self.deselect(d3.select(this));
        });
    };
};

DeselectableBehavior.prototype = {
    constructor: DeselectableBehavior
};

DeselectableBehavior.parameters = [
];

module.exports = DeselectableBehavior;
