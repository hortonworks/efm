...
describe('EFM Flow Resolver', function () {
    var flowResolver;
    var flowProvider;
    var efmService;
    ...
    beforeEach(function () {
        ...
        flowResolver = ngCoreTesting.TestBed.get(FlowResolver);
        flowProvider = ngCoreTesting.TestBed.get(FlowProvider);
        efmService = ngCoreTesting.TestBed.get(EfmService);
    });

    it('should resolve a flow', function () {
        // Spy
        spyOn(flowProvider, 'setFlowId').and.callThrough();
        ...
        flowResolver.resolve({
            paramMap: {
                get: function () {
                    return '2d6beca5-f1a4-4e63-9951-37b34c57c532';
                }
            }
        });

        //assertions
        expect(flowProvider.setFlowId).toHaveBeenCalled();
        expect(flowProvider.get).toHaveBeenCalled();
    });

    it('should fail to resolve a flow', function () {
        // Spy
        spyOn(efmService, 'routeToOpenFlow').and.callFake(function () {});
        spyOn(flowProvider, 'setFlowId').and.callThrough();
        spyOn(flowProvider, 'getFlowId').and.callThrough();
        spyOn(flowProvider, 'get').and.callFake(function () {
            return new Promise(function (resolve, reject) {
                reject({
        ...
        flowResolver.resolve({
            ...
            expect(efmService.routeToOpenFlow).toHaveBeenCalled();
            expect(flowProvider.setFlowId.calls.count()).toBe(2);
        });

        //assertions
        expect(flowProvider.setFlowId).toHaveBeenCalled();
        expect(flowProvider.setFlowId.calls.count()).toBe(1);
        expect(flowProvider.getFlowId).toHaveBeenCalled();
        expect(flowProvider.get).toHaveBeenCalled();
    });
});
