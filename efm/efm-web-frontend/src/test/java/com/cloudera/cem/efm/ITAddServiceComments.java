...
public class ITAddServiceComments {
    ...
    @Test
    public void testAddServiceComments() throws Exception {
        ...
        // confirm configure icon visible
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.className("fa-cog")));

        // select configure icon for service
        WebElement configureServiceButton = driver.findElement(By.className("fa-cog"));
        configureServiceButton.click();

        // wait for comment field to be visible
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("mat-input-1")));

        // place cursor in comment field
        WebElement serviceCommentInput = driver.findElement(By.id("mat-input-1"));
        serviceCommentInput.clear();

        // clear name field and enter ABC for name
        serviceCommentInput.sendKeys("ABC");

        // wait for Apply button to be visible
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("[data-automation-id='flow-designer-controller-service-configuration-apply-side-nav']")));

        // select Apply button
        WebElement applyButton = driver.findElement(By.cssSelector("[data-automation-id='flow-designer-controller-service-configuration-apply-side-nav']"));
        JavascriptExecutor executor = (JavascriptExecutor)driver;
        executor.executeScript("arguments[0].click();", applyButton);

        // wait for configure service side nav to close
        wait.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector("#flow-designer-controller-service-configuration")));
    }
    ...
    public void tearDown() throws Exception {
        ...
        executor2.executeScript("arguments[0].click();", deleteServiceButton);
